...
<script>
	$(function() {
		...
		var count2 = 0;
		$("#pw").keyup(function() {
			if($("#pw").val() == pw){ // 입력한 값과 db에서 가져온 값이 같다면
		...
		$("#pw2").keyup(function() {
			if($("#pw2").val() == "" || $("#pw2").val().length < 8){ //빈칸이 아니고 길이가 8자 미만이라면
		...
		$("#join").click(function() {
			if($("#id").val() != id) { // 입력한 값과 db에서 가져온 id 가 다르다면
				alert("아이디는 수정할 수 없습니다.")
				return false;
			} 
			if(count == 1 || $("#pw").val() == "" || count2 == 1){	// 비밀번호가 다르거나 빈칸이거나 8자 보다 짧다면		
				alert("정보를 확인해주세요.");
				return false;
			}
			if($("#pw2").val() == ""){ // 빈칸이라면
