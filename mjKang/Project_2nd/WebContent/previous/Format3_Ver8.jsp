...
//세션아이디 필요
	$(function() {
        		...
        		success: function(result) {	
					...
					if($(result).length>0){
						$(result).each(function(i, result) {
								...
								//console.log(state);
								statebtn='<button class="statechange">현재상태</button>';
								...
								+"<td>"+type1+"</td>"
								+"<td>"+address1+"</td>"
								//+"<td id='trstate'><button class='statechange'>현재상태"+i+"</button></td>";
								+"<td><div id='trstate'>"+statebtn+"</div></td>";
								//state는 버튼 들어가야하는 장소
								str+="</tr>";
								//각 data table 생성 및 넣기,테이블 생성 어떻게... for문으로 아래 연달아서????
								//, state에 따라 reservation confirm 버튼 diabled변경
								//$('#trstate').append('<button class="statechange">현재상태</button>');
							})//each	
								$("#reservationlist").append(str); //each이후 고정
							}//if
	 ...
	 //confirm
	$("#reservation-confirm").on("click",".statechange", function() {
